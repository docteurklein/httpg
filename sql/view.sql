        ...
        xmlelement(name div, xmlattributes('grid interest' as class), (
            select xmlagg(
                xmlelement(name article, xmlattributes('card' as class, format('%s-%s', interest.good_id, interest.person_id) as id),
                    ...
                    (with message_id (message_id) as (select uuidv7())
                    select xmlelement(name form, xmlattributes(
                        'POST' as method,
                        '/cpres/query' as action
                    ),
                        xmlelement(name input, xmlattributes(
                            'hidden' as type,
                            'sql' as name,
                            format('insert into cpres.message (message_id, good_id, person_id, content) values (%L, %L, %L, $1)', message_id, interest.good_id, interest.person_id) as value
                        )),
                        xmlelement(name input, xmlattributes(
                            'hidden' as type,
                            'redirect' as name,
                    ...
                    case when interest.state in ('approved', 'late', 'given')
                        ...
                        when not given then xmlelement(name form, xmlattributes(
                            ...
                            xmlelement(name input, xmlattributes(
                                'hidden' as type,
                                'sql' as name,
                                'call cpres.give($1::uuid, $2::uuid)' as value
                            )),
                            xmlelement(name input, xmlattributes(
                                'hidden' as type,
                                'redirect' as name,
