\set ON_ERROR_STOP on

insert into cpres.translation (id, lang, text) values
  ('Welcome ', 'fr', 'Bienvenue ')
, ('a little interested', 'fr', 'un peu')
, ('interested', 'fr', 'beaucoup')
, ('highly interested', 'fr', 'passionnément')
, ('%s is %s', 'fr', '%s est %s interessé')
, ('%s asks for %s', 'fr', '%s recherche %s')
, ('Not interested anymore', 'fr', 'Plus intéressé')
, ('Interested', 'fr', 'Intéressé')
, ('Send login challenge', 'fr', 'Envoyer un lien de connection')
, ('map', 'fr', 'carte')
, ('Search', 'fr', 'Rechercher')
, ('About', 'fr', 'A propos')
, ('activity', 'fr', 'Notifications')
, ('Receiving activity', 'fr', 'Mes demandes en cours')
, ('Giving activity', 'fr', 'Mes dons en cours')
, ('my goods', 'fr', 'Gérer mes biens')
, ('By %s', 'fr', 'Par %s')
, ('%s at %s: ', 'fr', '%s le %s: ')
, ('HH24:MI, TMDay DD/MM', 'fr', 'TMDay DD/MM à HH24:MI')
, ('at %s', 'fr', 'le %s')
, ('TMDay DD/MM', 'fr', 'TMDay DD/MM')
, ('query', 'fr', 'Requête')
, ('title', 'fr', 'Titre')
, ('Create alert', 'fr', 'Créer une alerte')
, ('Remove alert', 'fr', 'Supprimer cette alerte')
, (' from %s', 'fr', ' de %s')
, ('Send message', 'fr', 'Envoyer')
, ('New good', 'fr', 'Ajouter un nouveau bien')
, ('Existing goods', 'fr', 'Mes biens')
, ('Submit', 'fr', 'Enregistrer')
, ('Add file', 'fr', 'Ajouter ce fichier')
, ('X', 'fr', 'X')
, ('Remove this good', 'fr', 'Supprimer ce bien')
, ('Are you sure?', 'fr', 'En êtes-vous sûr?')
, ('Give to %s', 'fr', 'Donner à %s')
, ('Given to %s', 'fr', 'Donné à %s')
, ('Check your emails', 'fr', 'Vérifiez vos emails et cliquez sur le lien reçu.')
, ('Nothing yet.', 'fr', 'Rien à lister.')
, ('%s is waiting for you on %s', 'fr', '%s vous attend pour %s')
, ('new row for relation "good" violates check constraint "good_title_check"', 'fr', 'Le titre doit être non-vide')
, ('Saved successfully', 'fr', 'Changement enregistré')
, ('https://wa.me/%s?text=About giving %s', 'fr', 'https://wa.me/%s?text=A propos du don de %s')
, ('https://wa.me/%s?text=About receiving %s', 'fr', 'https://wa.me/%s?text=A propos du don de %s')
, ('duplicate key value violates unique constraint "person_phone_key"', 'fr', 'Une autre personne utilise déjà ce numéro')
, ('duplicate key value violates unique constraint "person_name_key"', 'fr', 'Une autre personne utilise déjà ce nom')
, ('duplicate key value violates unique constraint "person_phone_key"', 'en', 'Another person already uses this number')
, ('duplicate key value violates unique constraint "person_name_key"', 'en', 'Another person already uses this name')
, ('new row for relation "person" violates check constraint "person_name_check"', 'en', 'Connot have an invalid name: no @, not empty')
, ('new row for relation "person" violates check constraint "person_name_check"', 'fr', 'Ce nom est invalide (pas de @, pas vide)')
, ('new row for relation "person" violates check constraint "person_phone_check"', 'en', 'Connot have an empty phone number')
, ('new row for relation "person" violates check constraint "person_phone_check"', 'fr', 'Le numéro de téléphone ne doit pas être vide')
, ('new row for relation "good_media" violates check constraint "good_media_content_check"', 'en', 'The file must not exceed 1MB in size.')
, ('new row for relation "good_media" violates check constraint "good_media_content_check"', 'fr', 'Le fichier ne doit pas dépasser 1MB.')
, ('current bid at %s€', 'fr', 'D''autres offrent %s€.')
, ('propose ', 'fr', 'Proposer ')
, ('Price?', 'fr', 'Prix?')
, ('go with google maps', 'fr', 'ouvrir dans google maps')
, ('For %s€', 'fr', 'Pour %s€')
, ('findings.help', 'fr', '')
, ('notified', 'fr', 'Cette personne a été notifiée.')
, ('could not notify', 'fr', 'Impossible de notifier cette personne.')
, ('%s gave you %s', 'fr', '%s vous a donné %s')
, ('%s is interested by %s', 'fr', '%s est interessé par %s')
, ('New message from %s', 'fr', 'Nouveau message de %s')
, ('Logout', 'fr', 'Se déconnecter')
, ('Delete account', 'fr', 'Effacer mon compte')
, ('Winner', 'fr', 'Vainqueur')
on conflict (id, lang) do update
    set text = excluded.text
;


